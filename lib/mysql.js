...
MySQL.prototype.find = function (model, id, callback) {
    var idString = Number(id);
    if (isNaN(idString)) {
        idString = "'"+id+"'";
    }

    var sql = 'SELECT * FROM ' +
        this.tableEscaped(model) + ' WHERE ' + this.escapeName('id') + ' = ' + idString + ' LIMIT 1';
...
MySQL.prototype.destroy = function (model, id, callback) {
    var idString = Number(id);
    if (isNaN(idString)) {
        idString = "'"+id+"'";
    }
    ...
    var sql = 'DELETE FROM ' +
        this.tableEscaped(model) + ' WHERE ' + this.escapeName('id') + ' = ' + idString;
...
MySQL.prototype.save = function (model, data, callback) {
    var idString = Number(data.id);
    if (isNaN(idString)) {
        idString = "'"+data.id+"'";
    }

    var sql = 'UPDATE ' + this.tableEscaped(model) + ' SET ' + this.toFields(model, data) + ' WHERE ' + this.escapeName('id') + ' = ' + (this.schema.settings.slave ? ('"' + data.id + '"') : idString);
...
MySQL.prototype.updateOrCreate = function (model, data, callback) {
    ...
    Object.keys(data).forEach(function (key) {
        if (props[key] || key === 'id') {
            var k = '`' + key + '`';
            var v = mysql.toDatabase(props[key], data[key]);
...
MySQL.prototype.propertiesSQL = function (model) {
    var self = this;
    var sql = []
    var p = this._models[model].properties['id']

    if (p.type.name == "Number"){
        sql.push('`id` INT(11) NOT NULL AUTO_INCREMENT PRIMARY KEY');
    }
    else {
        sql.push('`id` ' + datatype(p) + ' NOT NULL PRIMARY KEY');
    }
